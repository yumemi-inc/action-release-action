...
(async () => {
  ...
  if (versionInfo === null) {
    (0, import_core.setFailed)("Unsupported version format. Use `vx.y.z` style.");
    (0, import_node_process.exit)(1);
  }
  const octokit = new import_rest.Octokit({
    baseUrl: import_github.context.apiUrl,
    auth: token
  });
  ...
  await (0, import_core.group)("Creating source tag", async () => {
    if (disableSourceTag) {
      ...
      await (0, import_exec.exec)("git", ["tag", `${version2}-src`]);
  ...
  await (0, import_core.group)("Initialising release branch", async () => {
    ...
    await (0, import_exec.exec)("git", ["init", "-b", releaseBranch], runInDist);
  ...
  await (0, import_core.group)("Committing changes", async () => {
    await (0, import_exec.exec)("git", ["add", "."], runInDist);
    await (0, import_exec.exec)("git", ["commit", "-m", `feat: Release ${version2}`], runInDist);
    await (0, import_exec.exec)("git", ["tag", "-f", `v${versionInfo.major}`], runInDist);
    await (0, import_exec.exec)("git", ["push", "-u", "origin", releaseBranch], runInDist);
  ...
  await (0, import_core.group)("Creating a release", async () => {
    ...
    if (!disableSourceTag) {
      const releaseNotes = await octokit.repos.generateReleaseNotes({
        ...
        previous_tag_name: latestRelease ? `${latestRelease.data.tag_name}-src` : void 0
      });
